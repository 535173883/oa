<template>
  ...
  <div class="User">
    ...
    <!-- 用户列表 -->
    <div class="content" v-show="activeIndex=='83'">
      <div class="table_top">
        <span style="line-height: 31px;">用户姓名 ：</span>
        <input type="text" v-model="content_v" placeholder="搜索用户姓名" style="width:100px;" />
        <el-button style="float:right" type="primary" @click="adduser()" v-has="831">添加</el-button>
        <el-button style="float:right" type="danger" @click="resetpassword()" v-has="832">重置密码</el-button>
        <!-- <el-button style="float:right" type="success" @click="resetvacation()" v-has="811">重置年假</el-button> -->
      </div>
      <!-- .slice((this.currentPage-1)*this.pageSize,this.currentPage*this.pageSize) -->
      <el-table
        :data="tableData_yh.filter(data => !content_v || data.username.toLowerCase().includes(content_v.toLowerCase())).slice((this.currentPage-1)*this.pageSize,this.currentPage*this.pageSize)"
        style="width: 95.8%;margin:0 2.1%;cursor:pointer;height:522px;overflow:auto"
        align="center"
        ref="tableData_yh"
      ...
      >
        ...
        <el-table-column label="手机号" align="center" width="230">
          <template slot-scope="scope">
        ...
        <el-table-column label="角色" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span style="text-decoration:underline;cursor:pointer">{{ scope.row.rid }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="140">
          <template slot-scope="scope">
            <div class="operation">
              ...
              <span
                style="color:#3399FF"
                @click.stop="handleEdit1(scope.$index, scope.row)"
                v-has="833"
              >修改</span>
              ...
              <span
                style="color:#FF5454"
                @click.stop="handleDelete1(scope.$index, scope.row)"
                v-has="834"
              >删除</span>
            </div>
          </template>
      ...
      <div class="block">
        ...
        <el-pagination
          ...
          :total="tableData_yh.filter(data => !content_v || data.username.toLowerCase().includes(content_v.toLowerCase())).length"
        ></el-pagination>
      </div>
      ...
      >
        <el-form label-width="100px">
          <el-form-item label="角色：" prop="duty">
            <el-select v-model="user_duty" placeholder="请选择" style="width:100%">
              <el-option
        ...
        <span slot="footer" class="dialog-footer">
          <el-button @click="user_canael()">取 消</el-button>
          <el-button type="primary" @click="user_queding()">确 定</el-button>
        </span>
      </el-dialog>-->
      <!-- 用户添加 -->
      <el-dialog
        ...
        :show-close="false"
      >
        <el-form :data="user" :model="user" ref="user" :rules="rules" label-width="100px">
          ...
          <el-form-item label="职务：" prop="duty" style="text-align:left">
          ...
          <el-form-item label="是否党员：" prop="politics_status" style="text-align:left">
            <el-radio v-model="user.politics_status" label="正式党员" border>正式党员</el-radio>
            <el-radio v-model="user.politics_status" label="预备党员" border>预备党员</el-radio>
            <el-radio v-model="user.politics_status" label="群众" border>群众</el-radio>
          </el-form-item>-->
          ...
          <!-- <el-form-item label="合同到期时间：" prop="end_time" style="text-align:left" id="hetongend_time">
            <el-radio label="具体日期" border @change="user_hetongtime = true">具体日期</el-radio>
            <el-date-picker
          ...
          <!-- <el-form-item label="员工备案：" prop="emp_record" style="text-align:left">
            <el-radio v-model="user.emp_record" label="协会" border>协会</el-radio>
            <el-radio v-model="user.emp_record" label="中心" border>中心</el-radio>
          </el-form-item>
          <el-form-item label="提职时间：" prop="pro_time">
            <el-date-picker
              v-model="user.pro_time"
              type="date"
              placeholder="选择日期"
              style="width:100%"
            ></el-date-picker>
          </el-form-item>
          <el-form-item label="职务工资：" prop="duty_salary">
            <el-input v-model="user.duty_salary"></el-input>
          </el-form-item>
          <el-form-item label="岗位工资：" prop="post_salary">
            <el-input v-model="user.post_salary"></el-input>
          </el-form-item>-->
        ...
        <div slot="footer" class="dialog-footer">
          <el-button @click="cancel('user')" style="width: 120px;">取 消</el-button>
          <el-button type="primary" @click="submit('user')" style="width: 120px;">确 定</el-button>
        </div>
      </el-dialog>
    </div>
    <!-- 角色管理 -->
    <div class="content" v-show="activeIndex=='84'">
      <div class="table_top">
        <span style="line-height: 31px;">角色名称 ：</span>
        <input type="text" placeholder="搜索角色名称" v-model="content_i" style="width:150px;" />
        <el-button type="primary" @click="add_user()" v-has="841">添加</el-button>
      </div>
      <el-table
        :data="user_list.filter(data => !content_i || data.roleName.toLowerCase().includes(content_i.toLowerCase())).slice((this.currentPage_1-1)*this.pageSize,this.currentPage_1*this.pageSize)"
        :model="tableData_js"
        ref="tableData_js"
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <div class="operation">
              ...
              <span style="color:#67C23A" @click="user_fenpei(scope.row.rid)" v-has="842">分配权限</span>
              <span style="color:#3399FF" @click="user_xiugai(scope.row.rid)" v-has="843">修改</span>
              <span
                style="color:#FF5454"
                @click="role_delete(scope.$index, scope.row)"
                v-has="844"
              >删除</span>
            </div>
          </template>
      ...
      <div class="block">
        <span class="demonstration">每页10条</span>
        <el-pagination
          :current-page.sync="currentPage_1"
          :page-size="10"
          layout="prev, pager, next, jumper"
          :total="user_list.filter(data => !content_i || data.roleName.toLowerCase().includes(content_i.toLowerCase())).length"
        ></el-pagination>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="剩余年假(天)：" prop="remainannualleave" label-width="106px">
          <el-input v-model="user.remainannualleave"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    searchMessage() {
      ...
      this.$http.post("/emp/gum", data).then((res) => {
        console.log(res.data);
        this.user.duty = Number(res.data.duty);
        this.user.mobile = res.data.mobile;
        this.user.workno = res.data.workno;

        this.user.utype.push(Number(res.data.utype));
        this.user.department.push(Number(res.data.department));
    ...
    peopleListmethods() {
      let data = new FormData();
      this.$http.post("/emp/gauu", data).then((res) => {
    ...
    endtime_filters() {
      ...
      this.$http.post("/uc/sau", data).then((res) => {
    ...
    handleEdit2(row, num) {
      ...
      this.$http.post("/uc/gu", data).then((res) => {
          ...
          if (valid) {
            ...
            this.$confirm("确认信息填写无误", "提示", {
              ...
              if (this.code == 100) {
                ...
                this.$http.post("/uc/a", data).then((res) => {
                  if (res.data == true) {
                    this.dialogFormVisible_1 = false;
                    this.dialogFormVisible = false;
                    console.log("新增", res);
                ...
                data.delete("lastLoginTime");
                this.$http.post("/uc/us", data).then((res) => {
                  if (res.data == true) {
                    this.dialogFormVisible = false;
                    this.dialogFormVisible_1 = false;
                    console.log("修改", res);
  ...
  mounted() {
    ...
    this.get_job();
    this.get_user();
    this.peopleListmethods();
    let a = JSON.parse(sessionStorage.getItem("a"));
    console.log(a);
    for (let item of a) {
      if (item == "83") this.menu.push({ name: "用户列表", id: "83" });
      // if (item == "81") this.menu.push({ name: "部门管理", id: "81" });
      if (item == "84") this.menu.push({ name: "角色管理", id: "84" });
      // if (item == "81") this.menu.push({ name: "用户列表", id: "81" });
      // if (item == "82") this.menu.push({ name: "角色管理", id: "82" });
      // if (item == "83") this.menu.push({ name: "部门管理", id: "83" });
      // if (item == "84") this.menu.push({ name: "员工管理", id: "84" });
    }
